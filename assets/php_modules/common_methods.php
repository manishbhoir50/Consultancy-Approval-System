...
function load_details($required, $role, $sdrn, $dept)
{
    ...
    if ($role == "faculty") {
        if ($required == "all")
            ...
            $query = "SELECT  `internship_id`, `Topic`, `status`, `Date_submission` FROM `internships` WHERE `Sdrn` = '$sdrn' AND `status` IN ('pending', 'approved by hod', 'approved by principle') ORDER BY `Date_submission` DESC, `internship_id` DESC";
        } else if ($required == "rejected") {
            $query = "SELECT  `internship_id`, `Topic`, `status`, `Date_submission` FROM `internships` WHERE `Sdrn` = '$sdrn' AND `status` IN ('rejected by din', 'rejected by hod', 'rejected by principle') ORDER BY `Date_submission` DESC, `internship_id` DESC";
        } else
        ...
        if ($required == "all")
            ...
            $query = "SELECT  `internship_id`, `Topic`, `status`, `Date_submission` FROM `internships` WHERE Sdrn IN ( SELECT Sdrn FROM faculty WHERE Department = '$dept') AND `status` IN ('pending', 'approved by hod', 'approved by principle')  ORDER BY `Date_submission` DESC, `internship_id` DESC";
        else if ($required == "rejected") {
            $query = "SELECT  `internship_id`, `Topic`, `status`, `Date_submission` FROM `internships` WHERE `Sdrn` = '$sdrn' AND `status` IN ('rejected by din', 'rejected by hod', 'rejected by principle') ORDER BY `Date_submission` DESC, `internship_id` DESC";
        }
         else
...
function upload_details($sdrn, $location, $topic, $consultancy, $company_name, $amount, $path, $count, $startdate, $enddate, $skill, $submission_date)
{
    ...
    $email = $res['Email'];
    $body = $sdrn.' have applied for'.$topic.'<br>for more details <a href = "localhost/internship-approval-master/faculty">click here</a>';
    sendMail($email, $topic.' is sent for approval', $body);
...
function Set_Details($role, $response, $internship_id, $email, $topic)
{
    ...
    <?php
            ...
            $query = "UPDATE `internships` SET  `status` = 'rejected' , 'Rejection_Reason'= $response WHERE `internship_id` = '$internship_id'";
            mysqli_query($conn, $query);
        }
    }
    ?>
